...
> 建立日期：2026-08-27　｜　最後修訂：2026-09-01
> 狀態：**v1 實作中的現行規格**（本文即現況，見第〇節「本文的維護方式」）

---

## 〇、本文件的定位

本文件是 **yt-space 唯一的現行規格**，取代先前兩份已作廢的設計（2026-07-30 的本機 `.mp4` pipeline、2026-08-07 的 clip 區間模型）。那兩份已從 `specs/` 移除，內容留在 git 歷史裡；它們之中仍然有效的部分**已全部併入本文**（技術事實見第二節，被排除的方案見附錄 A）。

改變的是**產品操作模型**，不是技術地基 —— 所有實測驗證過的技術事實原樣保留，因為它們描述的是 YouTube 與 Cloudflare 的客觀行為，與 UI 怎麼設計無關。
...
> 2026-09-01 依 [實作計畫](../plans/2026-09-01-yt-space-shot-v1-實作計畫.md)
> 的 11 項決定修訂：第五節的過濾相似與進度段數、第五節第二步的互動、
> 第六節的 Lightbox 主動作命名、第十一節的階段數，以及若干補充。逐條變更見 git。
...
因為 `recordingDetails.recordingDate` 多數為空（見第二節第 6 點），**「來自上傳日」才是常態**。既然是常態，就不該用警示的方式呈現 —— 見第五節第三步的時間欄位（⏳ 未定案）。它本來就是批次欄位，一支影片改一次就好。
...
每一步頂部有三段式進度指示，**帶文字**：`1. 貼網址 › 2. 挑畫面 › 3. 填資料`。
返回上一步不會丟失該步的選擇。

> 曾評估四段式（第四步先灰掉佔位，v2 上線不必改版）。否決：v1 只有三步，
> 先畫一格永遠灰著的第四步等於在每一次取圖時展示一個做不到的承諾。
> 代價是 v2 追加第四步時進度列要改版，已知並接受。

### 第一步：輸入網址

接受 `youtu.be/xxx`、`youtube.com/watch?v=`、帶 `&t=` 的網址、`youtube.com/shorts/`、以及純 videoId。

輸入框下方是**「最近取過的影片」清單**：影片圖示＋標題＋日期＋已取張數，**點一列直接進第二步**（不是把網址填回輸入框）。資料來自 `GET /api/videos/recent` —— 由 `video` 表推導（該 `owner_id` 依 `added_at` 排序，附各片的 shot 數），不另存一份歷史。

按【確定】後，`GET /api/video/{videoId}` 取回 metadata（Data API v3，`part=snippet,contentDetails,recordingDetails`）、storyboard spec，以及**該片已收藏的 `frame_index` 清單**（第二步要據此標示鎖定格）。可能的結果：
...
│                           │
├───────────────────────────┤
│ 已收斂成 50 張候選，藏了 98 張  [顯示全部] │  ← 收斂狀態列
├───────────────────────────┤
│ 全部選取│手動補圖│只看已選 │  ← 工具列，換行不截斷
├───────────────────────────┤
│ ┌────┐ ┌────┐ ┌────┐      │
│ │    │ │ ✓  │ │    │      │  ← 每列 3 張
...
| **點每格右上角的 ▶** | 同長按 —— 跳去播放。長按仍然有效，但**不是唯一入口**：鍵盤與輔助技術到不了長按。 |
| 已收藏的格子 | 點下去不會勾選，但要**明確提示「這一格已經收藏過了」**，不是靜靜地沒反應。長按與 ▶ 仍可跳去播放。 |

**決策理由**：挑圖是主要動作，必須拿到最順的手勢，而且不需要瞄準。曾評估「點＝播放、右上角小圓圈＝勾選」，但在 96px 寬的格子上小圓圈只有約 20px，選 30 張要精準點 30 次，否決 —— **注意這條否決的是「用小圓圈勾選」，不是「格子上有小按鈕」**：▶ 承載的是次要動作（播放），點不準的代價只是沒跳播，重點一次即可。

首次進入第二步時，在工具列下方顯示一次性提示：「點一下收藏・長按看看那一段」，點任意處消失，記在本機不再出現。

#### 收斂：進場時就已經過濾過

縮圖牆不是攤開 148 格讓使用者自己找，而是**一進第二步就已收斂成候選**，把連續相似的畫面藏起來。

- 頂部狀態列說明現況：「已收斂成 50 張候選，隱藏了 98 張相似畫面」，旁邊一顆【顯示全部】。
- 按【顯示全部】→ 148 張全部出現，按鈕變成【重新過濾】，可以來回切。
- **強度沿用帳號頁的「取圖 › 過濾相似強度」設定**（高／中／低，預設中），不在這一頁重複提供選項。切換設定後回到本頁，以新門檻對原始 148 格重算（不是在已過濾的結果上疊加）。
- 演算法：每格縮到 9×8 灰階，算 **dHash**（相鄰像素比較，得 64-bit 指紋），漢明距離小於門檻視為相似。依時間順序掃描，每組保留最早的一張。暫定門檻：高 ≤ 10、中 ≤ 6、低 ≤ 3，待實測調校（見第十六節）。
- 在**瀏覽器端**進行。sheet 經 `GET /api/sheet?u=…` 代理後變同源，canvas 即可讀取像素（見第二節第 4 點）。這些 sheet 在按【下一步】裁圖時本來就要下載，因此不算白做。

> **⚠️ 這是全案效能風險最高的一段。** 進場即收斂代表首屏前要下載**全部約 17 張 sheet**、
> 解碼、算 148 次 dHash，而不是只算使用者勾選的那幾張。
>
> **實作方式**：縮圖牆先直連 `i.ytimg.com` **立刻畫出 148 格**（不等 dHash），
> 指紋計算在 Web Worker 背景進行，算完才套用收斂。即使計算慢，使用者也不是對著白畫面等。
>
> **退路與判定時機**：見實作計畫的 R-1 —— 階段 4 以 Pixel 7 ＋ 4G 節流實測
> 「進入第二步 → 收斂完成」的 p75，**超過 5 秒就退回按鈕觸發式**。

> **曾評估並否決的三個方案**：
> ① **按鈕觸發、只比對已勾選的**（本文 2026-08-27 的原始設計）—— 首屏最快，
> 理由是「使用者已經表達過意圖的才需要降噪」。否決：那是把降噪放在挑完之後，
> 但降噪的價值正是**讓使用者不必從 148 格裡挑**。順序反了。保留為 R-1 的退路。
> ② **分組列出讓使用者逐組確認** —— 最保險，但多一個畫面、多一輪操作。
> ③ **勾選時即時提醒** —— 需頻繁比對，體驗零碎。
...
| 欄位 | 預設值 | 說明 |
|---|---|---|
| **時間**（`event_date`） | 見第四節的預設值鏈 | ⏳ **未定案**。首頁分組的依據，是這一步最重要的欄位。呈現方式兩案：(a) 欄位下方一行中性說明「預設帶入 YouTube 的日期，可以改成實際拍攝日」（驗收操作手冊要求，明確反對警告色）；(b) 標題旁橘色 ⚠️ 提示、視覺上比其他欄位更顯眼。因為「來自上傳日」是常態（第四節），(a) 較合理，實作暫依 (a)，待確認。 |
...
| **【▶ 播放這一段】** | 導向 `/v/{videoId}?t={at_sec}`，詳情頁播放器從該秒開始播。（曾命名「跳到影片時間」，太長；也曾定案「影片播放」，但驗收手冊採用「播放這一段」，以手冊為準。） |
...
| 一張圖可進幾個資料夾 | 不限（多對多） |
| 清單頁的卡片 | 名稱 ＋ **含子孫層**的計數 ＋ **內容預覽拼貼（最多 4 張）**；空資料夾顯示「還沒有圖片」。預覽需要每個資料夾的前 4 個 `thumb_key`，列表一次要 N×4 —— 這是 N+1 的風險點，作法與判定見實作計畫 R-3 |
| 清單頁的操作 | 名稱篩選字串 ＋ 四種排序（名稱升／降、張數多→少、最近加入）。「張數多→少」需先算出全部資料夾的含子孫計數再排序 |
...
```mermaid
flowchart LR
    H["首頁／查詢／分類<br/>點縮圖"] --> LB["Lightbox"] -->|播放這一段| V["詳情 /v/[videoId]"]
...
| 失敗 | 行為 |
|---|---|
| **解不出 spec**（影片太短／直播中／剛上傳，或解析器失效） | 第一步仍可進第二步，縮圖牆空白，只開放【手動補圖】。UI 明白提示「無法取得這支影片的逐段縮圖，可以手動補圖」 |
| **spec 解得出來，但 sprite 圖回 403**（`sigh` 簽章已過期，見第二節第 3 點） | 縮圖牆**退回顯示該影片的封面圖**（`i.ytimg.com/vi/{id}/hqdefault.jpg`，無簽章、不會過期），而不是一片破圖。使用者仍能靠時間標籤與播放器挑格 |

- **功能降級，絕不當機**
- **已存入 R2 的單格縮圖完全不受影響**，首頁與查詢照常運作；失效只影響「之後新加入的影片」與「取圖第二步的候選牆」
...
點縮圖 → **Lightbox**（見第六節）→【播放這一段】→ 詳情頁 iframe 跳到 `at_sec` 開始播放：
...
4. 我們的程式只做一件事：取出 email 當 `owner_id`（`apps/web/src/lib/server/auth.ts`，約十行）

**三個實作細節（比上面的敘述更省事，也有一個坑）：**

- **不需要自己驗 JWT。** Workers 的 `ctx.access.getIdentity()` 直接回傳 email 與 name。
- **只掛一個 IdP 時開啟 Apply instant authentication**，Cloudflare 自己的登入頁也會被跳過 —— 使用者開站就直接進 Google 帳號選擇器，站內完全沒有登入畫面。
- ⚠️ **SPA 的坑**：session 到期時，`fetch` 拿到的會是轉跳後的 HTML 而不是 JSON。所有 API 請求必須帶 `X-Requested-With: XMLHttpRequest`，Access 才會回 401，前端據此強制刷新。漏掉這條的症狀是「畫面隨機壞掉」。
- 本機與 E2E：`wrangler dev` 支援 `access.dev` 區塊，可假裝已登入，測試不會被閘門擋住。
...
**這一頁的兩個設定值（過濾相似強度、AI 分析區間）存在 `localStorage`，不進資料庫。** 第四節的資料模型因此沒有 `user_setting` 表。代價是換一台裝置會回到預設值 —— 對單人使用的 v1 可接受，日後要跨裝置同步再加表，不影響既有資料。

帳號區塊的版面沿用原型 `mockups/uiux-v2/account.html`：漸層 hero ＋ 跨在下緣的三格統計卡（收藏張數／本月新增／來源影片數），三格都是入口。統計數字來自 `GET /api/stats`；「本月新增」以 `event_date` 的年月比對，與首頁分組同一口徑，走 `shot(owner_id, event_date, id)` 索引即可，不需要彙總表。
...
| 8 | Lightbox：全屏檢視、左右滑動、播放這一段、加入分類、分享、編輯、刪除 | 圖庫的核心瀏覽體驗成形 |
...
| 12 | 帳號頁（含原設定項目增刪）、`/api/stats` 三格統計 | 設定與功能一致 |
| 12A | **標籤管理後端**：`/api/tags` 的列出／改名／改 kind／別名／刪除；刪除與改名需同步 `shot_tag` 與 `facet_month_agg` | 設定頁的標籤管理真的能改到資料 |
...
| 14A | **縮圖修復**：缺圖查詢 ＋【重新抓取缺少的縮圖】。⚠️ 裁切一律在瀏覽器（本節原則 4），所以補抓**不能在 Worker 或 probe 裡跑**，只能是前端驅動、分頁需保持開啟的循序作業，且必須可續做 | 降級期間入庫的影片補得回縮圖 |
...
14. 已收藏過的格子 → 顯示鎖定、點擊時出現「這一格已經收藏過了」提示，不靜默
...
20. Lightbox：左右滑動換張、【播放這一段】導向詳情頁且秒數正確、【刪除】後自動跳下一張
...
**v1 比 clip 版更省**：不做影片分析，Gemini 只用在查詢解析（純文字、極輕量）。clip 版估算的「500 clips ≈ 4.2 小時影片分析」整個消失。
...
- Lightbox 全屏檢視（左右滑動、播放這一段、加入分類、Web Share 分享、編輯、刪除）
- 分類資料夾（樹狀、深度 5、多對多收圖）
...
| 本機 `.mp4` pipeline | v2 | 與產品核心不同調，見附錄 A-3 |
| 使用者自帶 Gemini 金鑰 | v2 | 已驗證可行（附錄 A-2），但 v1 幾乎不用 Gemini，動機不足 |
| **脫離 Cloudflare 的自架版本** | **v3** | 使用者指定的長期選項。可行性評估與 v1 需預留的邊界見第十八節 |
| Google Drive + DuckDB 取代 D1/R2 | ❌ 不做 | 已評估並排除，見附錄 A-1 |
...
**選擇性是重點** —— 使用者收了 18 張圖，可能只有 5 張值得花 AI 額度。這是「節省 AI 分析資源」這個一路保留下來的目標的延續。
...
> 這些是**被取代的舊規格**（2026-07-30、2026-08-07）裡仍然有效的結論，於 2026-09-01
> 併入本文，兩份舊規格隨即從 `specs/` 移除（內容仍在 git 歷史）。
> 收錄的目的只有一個：**不要再花時間重新評估一次。** 完整的推導過程見 git。

### A-1 Google Drive + DuckDB 取代 D1/R2 —— ❌ 不做

> 評估日期 2026-08-26。動機是「把資料庫與儲存空間轉嫁到使用者自己的空間」，
> 以消除中央持有他人資料的問題與 D1 容量天花板。

- **DuckDB 是選錯工具。** `duckdb-wasm` 送不出 auth header（[duckdb-wasm#1967](https://github.com/duckdb/duckdb-wasm/issues/1967)），
  而 Drive 檔案必須帶 `Authorization` → 直接 range 讀 Drive 這條路是斷的。
  但本系統也**不需要** range 讀：500 shots ≈ 1 MB，整包下載不到一秒。
  一旦確定「整包下載到瀏覽器查詢」，DuckDB 就全面輸給 SQLite-WASM（bundle 小得多、
  本系統是 OLTP 小查詢而非 OLAP 欄式掃描，且 FTS5 trigram 可原封不動沿用）。
  → **若未來真要走「使用者自帶空間」，引擎應是 SQLite-WASM，不是 DuckDB。**
- **Drive 當資料庫後端可行但代價高。** 正面：媒體端點的 CORS 允許 `Range`（「Google 擋 Range」的說法已過時），
  讀寫端點 preflight 皆通過，scope 用 `drive.file`（非敏感、免審）即可。
  ⚠️ OAuth app 必須發佈到 Production —— Testing 狀態的 refresh token 7 天就過期。
  負面：**無並發控制**（Drive v3 的 File 資源已移除 `etag`，沒有可靠的樂觀鎖，
  兩台裝置同時寫會 lost update，唯一乾淨解是 append-only journal ＋ compaction，
  這層同步邏輯要自己寫自己測），且伺服器端搜尋消失、金絲雀無處可寫、彙總表失去意義。
- **縮圖搬 Drive 是決定性的否決理由。** R2 上一張圖是 `<img loading="lazy" src="...">`，
  零 JS、瀏覽器原生延遲載入、`immutable` ＋ CDN 快取（冷開機 0 請求）。
  Drive 必帶 `Authorization`，`<img>` 與 CSS 完全不能用，只能 fetch → blob → objectURL，
  於是要自建：IntersectionObserver ＋ 併發上限佇列、Cache Storage ＋ 淘汰策略、
  `revokeObjectURL` 的生命週期管理、token 到期的 401 → refresh → retry 收斂。
  約 200～400 行，而且是最難在手機上調對的那種。
  **換到的是 12 MB 儲存空間** —— 500 shots ≈ 3 MB，R2 免費額度 10 GB，需要 3000 倍的量才開始付費。

### A-2 使用者自帶 Gemini 金鑰 —— ✅ 已驗證可行，v1 不採用

> 評估日期 2026-08-26。**技術上可行、實測通過，保留為選項。**

- CORS 實測結果已併入本文第二節第 4 點的矩陣（Gemini、YouTube Data API、Drive 讀寫、
  GIS + PKCE 皆可從瀏覽器直呼）。
- **唯一的阻塞點是取得 storyboard spec 字串** —— 它只存在於 watch page HTML 的
  `playerStoryboardSpecRenderer`，而 watch page 與 InnerTube 都不給 CORS。
  改用 `hqdefault.jpg` 不成立（那是整支影片的封面，同片每張 shot 縮圖會長一樣）；
  公開 CORS proxy 不可靠且洩漏使用者看了哪些影片。
  → 即使採用自帶金鑰，架構仍是「**近乎零後端**」而非零後端：中央至少要留一個無狀態端點。
- **真正的代價是 onboarding 摩擦**：使用者得自己去 AI Studio 開專案、產金鑰、貼進設定頁。
  技術上零成本，產品上是一道會實質勸退人的牆。
- 金鑰會暴露在瀏覽器，但那是使用者自己的金鑰，爆炸半徑限於他自己的免費額度；
  referrer 限制靠不住，**至少要把該 key 限縮為只能呼叫 Generative Language API**。
- **v1 不採用的理由**：v1 幾乎不用 Gemini（只有查詢解析，純文字、極輕量），動機不足。
  列為 v2 選項（見第十五節）。

### A-3 本機 `.mp4` pipeline（2026-07-30 的原始構想）—— 擱置至 v2

最早的構想是處理自己拍的 `.mp4`：本機轉檔、抽幀、建索引。
與現在的產品核心（**收別人的 YouTube 影片畫面**）不同調，且需要一整套本機執行環境。
擱置，未排入 v1 或 v2 的確定範圍。
