...
| **D-27** | 第三步時間欄位：手冊要求「下方是中性說明『預設帶入 YouTube 的日期，可以改成實際拍攝日』，**不是**欄位標題旁邊的橘色 ⚠ 警告」；規格第五節則要求「來自上傳日時顯示 ⚠️ 提示」「視覺上比其他欄位更顯眼」 | **依手冊，用中性說明** | 這是 UI 表達方式，手冊是 UI 的目標狀態；規格在意的是「使用者要知道這可能不是拍攝日」，中性說明同樣達成 |
...
| T2.5 | 第二步互動：點＝勾選、長按 0.5s＝跳播（震動 15ms）、**每格右上角 ▶**（D-20）、已收藏格鎖定並提示（D-19） | M | 手冊 §四第二步：「**每格右上角有一顆 ▶**，點了跳到那一段；長按仍然有效，但不是唯一入口 —— 鍵盤與輔助技術也要到得了。」「已收藏過的格子點下去會提示『這一格已經收藏過了』，不是靜靜地沒反應。」<br>**▶ 與提示都是新功能**，規格第五節只有點／長按與「點擊無效」 |
...
| T3.5 | **sprite 回 403（簽章過期）時退回封面圖** **（D-18）** | S | 手冊 §九：「sprite 網址簽章過期時會退回封面圖，這是預期中的降級。」<br>**與「解不出 spec」是兩種不同的失敗**，規格第七節只定義了後者 |
...
⚠️ **這一段有個規格沒處理到的結構問題**：裁切一律在瀏覽器（規格第三節原則 4、第七節），
...
規格第五節明文寫的是「只比對已勾選的，不比對全部 148 張」，選 ① 等於推翻這條約束，而這條約束原本正是為了避免這個成本。
...
**選了 Cloudflare Access 之後，測試人員照現行手冊驗會有一整節 fail。**
手冊是驗收標準，這些條目必須先改，否則階段 13 沒有辦法驗收。
